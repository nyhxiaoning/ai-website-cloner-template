...
export default function CreateSnippetDialog({
  ...
  const [type, setType] = useState('自定义');
  const [suggestedPosition, setSuggestedPosition] = useState('中段');
  ...
  const handleConfirm = () => {
    ...
    onConfirm({ key: key.trim(), title: title.trim(), content: content.trim(), type, suggestedPosition, tags: tags.trim() });
    reset();
    onOpenChange(false);
  };
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <FormField label="建议位置" htmlFor="snip-position">
            <select
              ...
              onChange={(e) => setSuggestedPosition(e.target.value)}
            >
